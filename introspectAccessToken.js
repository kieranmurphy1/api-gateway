...
function validateAccessToken(r) {
    ...
    r.log("validateAccessToken(): REMOTE_ADDR = " + r.remoteAddress);ss    
    if (r.variables.validateTokenFlag == "true") {
        ...
        if (parts.length === 2) {
            ...
            r.return(401, "validateAccessToken(): Authorization header does not have 2 parts");
            r.log("validateAccessToken(): 401 Error - Auth0rization header does not have 2 parts");
        }
    }
    else {
        r.log("validateAccessToken(): r.variables.validateTokenFlag = " + r.variables.validateTokenFlag);
...
function validateAccessTokenCache(r) {
    ...
    if (validateTokenFlag == "true") {
        if (r.headersIn.authorization && r.headersIn.authorization.length > 0) {
            var token;
            var parts = r.headersIn.authorization.split(' ');
                ...
                if (/^Bearer$/i.test(scheme)) {
                    ...
                    if (r.variables.token_cache && r.variables.use_jwt_token_cache == "true") {
                        ...
                        try {
                            ...
                            r.return(500, "validateAccessTokenCache(): Error in response from function tokenResult() - " + error + "\n");
                        }                        
                    } else {
                        ...
                        r.log("validateAccessTokenCache(): NJS subrequest: body - " + token);
                        r.subrequest("/_oauth2_send_request",
                            {
                                method: 'POST',
                                body: token
                            ...
                            function (reply) {
                                if (reply.status == 200) {
                                    ...
                                    if (response.active == true) {
                                        ...
                                        r.return(401, "validateAccessTokenCache(): JWT 200 RESPONSE validation = false " + reply.uri + "\n");
                                    }

                                } else {
                                    r.log("validateAccessTokenCache(): Error in response " + reply.uri + " " + reply.status.toString() + " " + reply.responseBody);
                                    r.return(reply.status, "validateAccessTokenCache(): Error in response " + reply.uri + "\n");
                                    //r.return(401); // Unexpected response, return 'auth required'
                                }
                            }
                        ); // end of r.subrequest
                    ...
                    r.log("validateAccessTokenCache(): 401 Error - Authorization header missing Bearer label");
                    r.return(401, "validateAccessTokenCache(): Authorization header missing Bearer label");                    
                }
            } else {
                r.log("validateAccessTokenCache(): 401 Error - Auth0rization header does not have 2 parts");
                r.return(401, "validateAccessTokenCache(): Authorization header does not have 2 parts");                
            }
        } else {
            r.log("validateAccessTokenCache(): 401 Error - Authorization header missing or empty string.");
            r.return(401, "validateAccessTokenCache(): Authorization header missing or empty string.");
        ...
        r.log("validateAccessTokenCache(): skip token validation - tokenTrueTest = " + tokenTrueTest);        
        try {
            r.log("validateAccessTokenCache(): JWT token added to cache, calling tokenResult().....");
            tokenResult(r); // Current response now in key-value store so use cache for validation, if true call createapikeyCache().
        } catch (error) {
...
function tokenResult(r) {
    ...
    if (response.active == true) {
        try {
            ...
            r.return(500, "tokenResult(): Error in response from function createapikeyCache() - " + error + "\n");            
        }        
    } else {
...
function createapikeyCache(r) {
    ...
    if (r.variables.maximo_apikeys_cache && use_maximo_apikeys_cache == "true") {
        ...
        try {
            ...
            r.return(500, "createapikeyCache(): Error in response from function parseApikey() - " + error + "\n");            
        }  
    } else {
        ...
        try {
            ...
            r.return(500, "createapikeyCache(): Error in response from function createapikeySync() - " + error + "\n");
        }
    }
}
//--------------------------------------------------------
function parseApikey(r) {
    ...
    if (response.apikey.length > 0) {
        ...
        r.return(401);  // API Key not found, return forbidden code
    }
}
//--------------------------------------------------------
...
function createapikeyAsync(r) {
    ...
    r.subrequest("/getmaximoapikey",
        ...
        function (reply) {
            if (reply.status == 200) {
                var response_getmaximoapikey = JSON.parse(reply.responseBody);
                        ...
                        r.subrequest("/deletemaximoapikey",
                            ...
                            function (reply) {
                                if (reply.status == 200 || reply.status == 204) {
                                    r.log("createapikeyAsync(): Deleted existing user APIKEY " + r.variables.maximoapikeyref);
                                } else {
                                    r.log("createapikeyAsync(): Failed to delete existing user APIKEY " + r.variables.maximoapikeyref);
                    ...
                    function (reply) {
                        if (reply.status == 200) {
                            ...
                            if (response_getmaximoapikey.member.length = 0) {
                                ...
                                r.log("createapikeyAsync(): MAXIMO USER APIKEYS DELETE NOT COMMITTED - response.member.length = " + response_getmaximoapikey.member.length + " ASSIGN WAIT INTERVAL = " + deleteCommitDelayMillisec);
                            }
                        }
                    } // end of subrequest function(reply) statement block.
                ); // end of r.subrequest function.
                    ...
                    r.subrequest("/createmaximoapikey_admin",
                        ...
                        function (reply) {
                            if (reply.status == 200 || reply.status == 201) {
                                var response_createmaximoapikey_admin = JSON.parse(reply.responseBody);
                                r.log("createapikeyAsync(): Created APIKEY for USER: " + response_createmaximoapikey_admin.userid + " - " + response_createmaximoapikey_admin.apikey);
                                ...
                                r.log("createapikeyAsync(): Added user APIKEY to NGINX cache - " + r.variables.maximo_apikeys_cache);
                                ...
                                r.log("createapikeyAsync(): Failed to create user APIKEY " + response_createmaximoapikey_admin.href);
                                r.log("createapikeyAsync(): Error in response from subrequest " + reply.uri + " " + reply.status.toString() + " " + reply.responseBody);
                                r.return(reply.status, "Error in response from subrequest " + reply.uri + "\n");
                            }
                        } // end of subrequest function(reply) statement block.
...
function _getapikey(r) {
    ...
    r.subrequest("/getmaximoapikey",
        ...
        function (reply) {
            if (reply.status == 200) {
                var response_getmaximoapikey = JSON.parse(reply.responseBody);
                r.log("_getapikey(): r.headersOut.SetCookie = " + r.headersOut.SetCookie);  // Check if WQebSphere is creating cookie for client browser.      
                if (response_getmaximoapikey.member.length > 0) {
                    r.log("_getapikey(): MAXIMO USER APIKEYS FOUND - response_getmaximoapikey.member.length = " + response_getmaximoapikey.member.length);
                    r.log("_getapikey(): Existing Maximo API Key href = " + response_getmaximoapikey.member[0].href);
                    r.log("_getapikey(): Existing Maximo API Key apikeytokenid = " + response_getmaximoapikey.member[0].apikeytokenid);
                    ...
                    r.log("_getapikey(): NO MAXIMO USER APIKEYS FOUND - response_getmaximoapikey.member.length = " + response_getmaximoapikey.member.length);
                    r.error("_getapikey(): NO MAXIMO USER APIKEYS FOUND - response_getmaximoapikey.member.length = " + response_getmaximoapikey.member.length);
                    r.return(reply.status, "_getapikey(): NO MAXIMO USER APIKEYS FOUND - response_getmaximoapikey.member.length = " + response_getmaximoapikey.member.length + "\n"); // API Key Empty List, return success code.                    
                }
            } else {
                r.log("_getapikey(): Error in response from subrequest " + reply.uri + " " + reply.status.toString() + " " + reply.responseBody);
    ...
    try {
        let res = r.subrequest("/deletemaximoapikey",
            ...
            function (reply) {
                ...
                res_uri = reply.uri;
                if (reply.status == 200 || reply.status == 204) {
                    r.log("_deletekey(): Deleted existing user APIKEY " + r.variables.maximoapikeyref);
                    //r.return(reply.status, "_deletekey(): Deleted existing user APIKEY " + r.variables.maximoapikeyref + "\n");
                } else {
                    r.log("_deletekey(): Failed to delete existing user APIKEY " + r.variables.maximoapikeyref);
                    r.log("_deletekey(): Error in response from subrequest " + reply.uri + " " + reply.status.toString() + " " + reply.responseBody);
                    //r.return(reply.status, "Error in response from subrequest " + reply.uri + "\n");
                }
            } // end of subrequest function(reply) statement block.
        ); // end of r.subrequest function.
        r.return(200, "_deletekey(): Deleted existing user APIKEY " + r.variables.maximoapikeyref + "\n");
        //return res;
    } catch (ex) {
        r.log("_deletekey(): Error in response from subrequest " + res_uri);
        //r.return(500, "Error in response from subrequest " + res_uri + "\n");
        return ex;
    }
}
//--------------------------------------------------------
    ...
    r.subrequest('/getmaximoapikey',{method: 'GET', body: ''})
        .then(reply => {
            if (reply.status != 200) {
                ...
                throw new Error("createapikeySync():/getmaximoapikey - subrequest failed for MAXIMO-USER-ID = " + r.headersIn['MAXIMO-USER-ID'] + ", reply.status = " + reply.status.toString());
                r.return(reply.status, reply.responseBody, "subrequest /getmaximoapikey failed.");
            } else {
                if (reply.responseBody.length > 0) {
                    ...
                    throw new Error("createapikeySync():/getmaximoapikey - reply body is empty for MAXIMO-USER-ID = " + r.headersIn['MAXIMO-USER-ID']);
                }            
            }
        })
        .then(deleteflag => {
                        ...
                        r.subrequest('/createmaximoapikey_admin', {method: 'POST', body: ''})
                            .then(reply => {
                                if (reply.status != 200 && reply.status != 201) {
                                    ...
                                    var response_createapikey = JSON.parse(reply.responseBody);
                                    ...
                                    var apikeyscache_json = JSON.parse(r.variables.maximo_apikeys_cache);
                            ...
                            .catch(e => r.return(500, e));  // End of subrequest /createmaximoapikey_admin as semicolon.                
                    })
                    .catch(e => r.return(500, e));  // End of subrequest /deletemaximoapikey as semicolon.                   
            } else {
        ...
        .catch(e => r.return(500, e));      // End of sub request /getmaximoapikey
}
//--------------------------------------------------------
        ...
        .then(reply => {
            if (reply.responseBody.length > 0) {
                ...
                if (response.member.length > 0) {
                    ...
                    deleteflag = 0;   // There is no existing apikey to delete.
                    return deleteflag;
                    //throw new Error("deleteapikeySync(): JSON OBJ - response.member.length = " + response.member.length);
                }
            } else {
                r.log("deleteapikeySync():/getmaximoapikey - reply.responseBody.length = " + reply.responseBody.length);
                throw new Error("deleteapikeySync():/getmaximoapikey - reply body is empty for MAXIMO-USER-ID = " + r.headersIn['MAXIMO-USER-ID']);
            }
        })
        .then(deleteflag => {
            if (deleteflag == 1) {
                ...
                r.subrequest('/deletemaximoapikey', {method: 'DELETE', body: ''})
                    ...
                    .catch(e => r.return(500, e));  // End of subrequest /deletemaximoapikey                    
            } else {
